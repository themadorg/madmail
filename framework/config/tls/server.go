...
func TLSDirective(m *config.Map, node config.Node) (interface{}, error) {
	...
	// Try to get a default configuration to use for root fields
	// (e.g. for clients that don't support SNI).
	defaultCfg, _ := cfg.Get()
	if defaultCfg == nil {
	...
	return &tls.Config{
		...
		CurvePreferences: defaultCfg.CurvePreferences,
...
func readTLSBlock(globals map[string]interface{}, blockNode config.Node) (*TLSConfig, error) {
	baseCfg := tls.Config{
		// Workaround for issue https://github.com/themadorg/madmail/issues/730
