...
**cmdeploy / `554 5.7.1 No DKIM signature found`:** Madmail must sign outbound federation (fixed in #146). Publish `default._domainkey` from `{state_dir}/dkim/default.txt`. Same-stack Madmail↔Madmail does not require that TXT. See [DNS and Mail Authentication](./12-dns-mail-auth.md).
