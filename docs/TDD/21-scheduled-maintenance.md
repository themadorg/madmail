...
| Directive | Field | Job |
|-----------|-------|-----|
| `retention 24h` | `AppConfig.retention` | `prune-old-messages` — delete files older than duration (by mtime) |
...
**Database toggle (no static config):**
...
| Crate | Role |
|-------|------|
| `chatmail-tasks` | `MaintenanceConfig`, `TaskId`, `run_task`, `spawn_maintenance_scheduler` |
...
|---------|-------------|------------------|--------|
| `prune-old-messages` | `retention`, `prune-messages` | `storage.imapsql retention` | `purge_mail_blobs_older` |
...
|------|----------|--------------|
| Periodic (retention + unused accounts) | 1 hour | No `retention` and no `unused_account_retention` in config |
| Auto-purge seen | 15 seconds | `__AUTO_PURGE_SEEN__` not `enabled` |

First tick runs after one full interval (Madmail ticker behavior).
