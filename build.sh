...
install() {
	...
	# Install delta patch files if they exist (generated by CI or generate_delta make target).
	if [ -f "${builddir}/madmail.patch" ]; then
		command install -m 0644 "${builddir}/madmail.patch" "${destdir}/${prefix}/bin/madmail.patch"
		if [ -f "${builddir}/madmail.patch.version" ]; then
			command install -m 0644 "${builddir}/madmail.patch.version" "${destdir}/${prefix}/bin/madmail.patch.version"
		fi
		echo "-- Installed delta patch files." >&2
	fi
