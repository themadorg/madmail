...
// Test in-memory storage delivery
func TestInmemoryDelivery(tt *testing.T) {
	tt.Parallel()
	t := tests.NewT(tt)
	...
	smtpConn.Writeln("Subject: Hi from in-memory!")
	smtpConn.Writeln("")
	smtpConn.Writeln("Hi! This is a test message using in-memory storage.")
	smtpConn.Writeln(".")
	smtpConn.ExpectPattern("2*")
	...
	imapConn.Expect("Subject: Hi from in-memory!")
	imapConn.Expect("")
	imapConn.Expect("Hi! This is a test message using in-memory storage.")
	imapConn.Expect(")")
	imapConn.ExpectPattern(`. OK *`)
}

// Test delivery to multiple recipients with in-memory storage
func TestInmemoryMultipleRecipients(tt *testing.T) {
	tt.Parallel()
	t := tests.NewT(tt)
...
func TestInmemqueueDelivery(tt *testing.T) {
	tt.Parallel()
	t := tests.NewT(tt)
	...
	smtpConn.Writeln("From: <sender@maddy.test>")
	smtpConn.Writeln("To: <testusr@maddy.test>")
	smtpConn.Writeln("Subject: Queued message test")
	smtpConn.Writeln("")
	smtpConn.Writeln("This message was queued before delivery.")
	smtpConn.Writeln(".")
	smtpConn.ExpectPattern("2*")
