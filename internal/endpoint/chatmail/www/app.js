function app() {
  return {
    ...
    // Channels (groups/broadcasts — mirrors chatmail-core securejoin group invite)
    channels: [],
    ...
    async init() {
      ...
      AppLog.info('App', 'Initialising application', { origin: this.serverUrl });

      // Sync AppLog with UI
      AppLog.onUpdate((logs) => { this.debugLogs = [...logs]; });

      // Initialize transport layer
      AppTransport.init({ serverUrl: this.serverUrl, email: '', password: '' });
      await this.openDB();
      const restored = await this.restoreState();
      if (restored) {
        // Restored from IndexedDB — update transport credentials and reconnect
        AppTransport.setCredentials(this.credentials.email, this.credentials.password);
        this.state = 'ready';
        AppLog.success('App', 'Session restored', { email: this.credentials.email });
        this.toast('success', `Restored session: ${this.credentials.email}`);
        this.connectWebSocket();
      } else {
        AppLog.info('App', 'No existing session found, moving to connect/register');
        await this.connect();
      }
    },
    ...
    async copyLogs() {
      try {
        const text = AppLog.getCopyableText();
        await navigator.clipboard.writeText(text);
        this.toast('success', 'Logs copied to clipboard!');
      } catch (e) {
        this.toast('error', 'Clipboard access failed');
      }
    },
    ...
    async restoreState() {
      ...
      try {
        ...
        this.channels = (data.channels || []).map(ch => ({ ...ch, unread: 0 }));
        ...
        AppLog.warn('App', 'Restore failed, will re-register: ' + e.message);
        return false;
      }
    },
    ...
    // ---- API helpers (delegated to AppTransport) ----
    async api(method, path, body) {
      return AppTransport.api(method, path, body);
    },
    ...
    async connect() {
      ...
      this.error = '';
      try {
        await this.sleep(1500); // Give server a moment to settle if just restarted
        ...
        AppTransport.setCredentials(creds.email, creds.password);
        ...
        this.state = 'setup';
        AppLog.error('App', 'Connect/Register failed: ' + e.message, e.stack);
      } finally {
    ...
    // ---- Channels (Group Invite) ----
    // Mirrors chatmail-core/src/securejoin.rs get_securejoin_qr() with group params

    /**
     * Create a new channel and generate its invite QR code.
     * Uses the same URL format as Delta Chat:
     * https://i.delta.chat/#FP&x=GRPID&i=INVITE&s=AUTH&a=ADDR&n=NAME&g=GRPNAME
     */
    async createChannel(name) {
      if (!name || !name.trim()) {
        this.toast('error', 'Channel name is required');
        return;
      }
      name = name.trim();
      const grpId = this.generateId(); // x= param (like chat.grpid in core)
      const inviteNumber = this.generateId(); // i= param
      const auth = this.generateId(); // s= param
      const fp = this.pgpFingerprint;
      const email = this.credentials.email;
      const displayName = (this.displayName || '').substring(0, 16);

      // Build invite URI matching securejoin.rs broadcast format
      // OutBroadcast uses b= (broadcast name) and j= (invite number)
      const channelNameEncoded = encodeURIComponent(name).replace(/%20/g, '+');
      const addrEncoded = encodeURIComponent(email);
      const nameEncoded = encodeURIComponent(displayName).replace(/%20/g, '+');
      const inviteUri = `https://i.delta.chat/#${fp}&x=${grpId}&j=${inviteNumber}&s=${auth}&a=${addrEncoded}&n=${nameEncoded}&b=${channelNameEncoded}`;

      let qrDataUrl = '';
      try {
        qrDataUrl = await this.generateQRDataUrl(inviteUri);
      } catch (e) { /* no QR image */ }
      ...
      const channel = {
        ...
        createdBy: email,
        isBroadcast: true,
        // OutBroadcast: owner can send messages to all subscribers
        role: 'owner',
        status: 'joined',
        members: [email],
        messages: [],
      ...
      this.newChannelName = '';
      this.saveState();
      this.toast('success', `Channel "${name}" created`);
      this.selectChannel(channel);
      return channel;
    },

    /**
     * Check if the current user owns the selected channel.
     * Mirrors chatmail-core: OutBroadcast = can send, InBroadcast = read-only.
     */
    isChannelOwner(channel) {
      if (!channel) return false;
      return channel.role === 'owner';
    },
    ...
    selectChannel(channel) {
      ...
      this.selectedContact = null; // deselect contact
      channel.unread = 0;
      this.saveState();
    },
     ...
     * NOTE: This is a method, NOT a getter. Alpine.js getters that return
     * new array references cause infinite re-render loops.
     */
    getChannelMessages() {
      if (!this.selectedChannel) return [];
      const chId = this.selectedChannel.id;
      return this.allMessages.filter(m => m.channelId === chId)
    ...
    deleteChannel(channel) {
      ...
      this.allMessages = this.allMessages.filter(m => m.channelId !== channel.id);
      this.saveState();
      this.toast('info', `Channel "${channel.name}" deleted`);
    },
    ...
    async handleSecureJoinScan(uri) {
      try {
        ...
        //   or with group: ...&x=GRPID&g=GRPNAME
        //   or with broadcast: ...&x=GRPID&b=BCASTNAME&j=INVITE
        let fragment;
        ...
        // For historic reasons, broadcasts use j instead of i for the invitenumber
        const inviteNumber = params.i || params.j;
        const auth = params.s;
        const inviterName = params.n || '';
        const grpId = params.x;
        const grpName = params.g; // group name
        const broadcastName = params.b; // broadcast channel name

        if (!inviterEmail || !inviteNumber) {
          throw new Error('Missing required fields in QR code');
        }

        // ---- Channel/Group Invite (has x= with g= or b=) ----
        if (grpId && (grpName || broadcastName)) {
          const channelName = grpName || broadcastName;
          const isBroadcast = !!broadcastName;

          // Check if we already joined this channel
          const existing = this.channels.find(ch => ch.id === grpId);
          if (existing) {
            this.toast('info', `Already in channel "${existing.name}"`);
            this.selectChannel(existing);
            return;
          }

          this.toast('info', `Joining ${isBroadcast ? 'broadcast' : 'group'}: ${channelName}...`);
          ...
          // Create a local channel entry — InBroadcast (subscriber, read-only)
          const channel = {
            id: grpId,
            name: channelName,
            inviteUri: uri,
            ...
            isBroadcast,
            // InBroadcast: subscriber — read-only, cannot send
            role: 'subscriber',
            status: 'joining', // Mark as joining until handshake completes
            members: [inviterEmail, this.credentials.email],
            messages: [],
            unread: 0,
          };

          try {
            channel.qrDataUrl = await this.generateQRDataUrl(uri);
          } catch (e) { /* no QR */ }

          this.channels.push(channel);

          // Send the SecureJoin request (vg-request for groups)
          await this.sendSecureJoinRequest(inviterEmail, inviteNumber, grpId);

          this.selectChannel(channel);
          this.saveState();
          this.toast('success', `Joined channel "${channelName}"`);
          return;
        }

        // ---- Plain contact verification (no group) ----
    ...
    async sendSecureJoinRequest(toEmail, inviteNumber, grpId) {
      ...
      await AppTransport.sendMail(this.credentials.email, [toEmail], rawEmail.join('\r\n'));
    },
    ...
    async deleteForEveryone(msg) {
      ...
      try {
        ...
        await AppTransport.sendMail(this.credentials.email, [msg.peerEmail], rawEmail.join('\r\n'));
    ...
    async sendMessage() {
      // ---- Channel broadcast (owner sends to all members) ----
      if (this.selectedChannel && this.isChannelOwner(this.selectedChannel)) {
      ...
      try {
        ...
        await AppTransport.sendMail(this.credentials.email, [to], rawEmail.join('\r\n'));
    ...
    // ---- Channel Broadcast Send (OutBroadcast owner -> all members) ----
    // Mirrors Delta Chat: OutBroadcast sends a separate copy to each member
    async sendChannelMessage() {
      const channel = this.selectedChannel;
      if (!channel || !this.isChannelOwner(channel)) return;
      ...
      try {
        ...
        const recipients = (channel.members || []).filter(
          m => m.toLowerCase() !== this.credentials.email.toLowerCase()
        );

        if (recipients.length === 0) {
          this.toast('info', 'No subscribers yet — message saved locally');
        }

        // Send to each member individually (broadcast = separate email to each)
        for (const to of recipients) {
          const contact = this.contacts.find(c => c.email === to.toLowerCase());
          const msgId = `<${this.generateId()}@${this.credentials.email.split('@')[1]}>`;
          const now = new Date().toUTCString();

          let messageBody;
          const encOpts = { from: this.credentials.email, to };
          if (contact?.pgpKey) {
            messageBody = await this.encryptMessage(text, contact.pgpKey, encOpts);
          } else {
            messageBody = await this.encryptMessage(text, null, encOpts);
          }
          ...
          const rawEmail = [
            ...
            `Subject: [...]`,
            `Chat-Version: 1.0`,
            `Chat-Group-Name: ${channel.name}`,
            `Chat-Group-ID: ${channel.id}`,
          ];

          rawEmail.push(this.buildAutocryptHeader());

          rawEmail.push(
            `Content-Type: multipart/encrypted; protocol="application/pgp-encrypted"; boundary="pgp-bcast"`,
            `MIME-Version: 1.0`,
            '',
            '--pgp-bcast',
            'Content-Type: application/pgp-encrypted',
        ...
        // Add to local view with channelId
        const sentUID = Date.now();
        this.allMessages.push({
          uid: sentUID,
          text,
          ...
          outgoing: true,
          encrypted: true,
          peerEmail: this.credentials.email,
          channelId: channel.id,
          rfc724mid: `<${this.generateId()}@${this.credentials.email.split('@')[1]}>`,
        });
        this.processedUIDs.add(sentUID);

        await this.saveState();
        this.$nextTick(() => this.scrollToBottom());
        this.toast('success', `Broadcast to ${recipients.length} subscribers 🔒`);

      } catch (e) {
        this.toast('error', 'Broadcast failed: ' + e.message);
        this.composeText = text; // restore
      } finally {
        this.sending = false;
      }
    ...
    // ---- WebSocket real-time messages (delegated to AppTransport) ----
    connectWebSocket() {
      this.polling = true;
      const wsOpts = {
        mailbox: 'INBOX',
        sinceUID: this.lastUID,
        onMessage: async (msg) => {
          AppLog.debug('App', 'Processing message from WS', { uid: msg.uid });
          try {
            if (msg.uid > this.lastUID) {
              this.lastUID = msg.uid;
              wsOpts.sinceUID = this.lastUID; // update for reconnects
            }
            await this.processIncoming(msg);
          } catch (e) {
            AppLog.error('App', 'WS message processing failed: ' + e.message, e.stack);
          }
        },
        onStatusChange: (status) => {
          AppLog.info('App', 'WebSocket status change: ' + status);
          if (status === 'connected') {
            AppLog.info('App', 'WebSocket connected');
            this.toast('success', 'Real-time connected');
          } else if (status === 'disconnected') {
            AppLog.info('App', 'WebSocket disconnected');
            this.toast('warn', 'Real-time disconnected');
          }
        },
      };
      AppTransport.connectWebSocket(wsOpts);
    },
    ...
    async processIncoming(msg) {
      ...
      // Skip realtime channel messages — they are handled by AppTransport
      if (AppTransport.isChannelMessage(msg.body)) return;
      ...
      try {
        ...
        const full = msg;
        AppLog.debug('Message', 'Processing incoming UID ' + full.uid, { envelope: full.envelope });
        ...
        if (peerEmail && peerEmail !== this.credentials.email.toLowerCase()) {
          ...
          const sjInviteNumber = effectiveHeaders['secure-join-invitenumber'];
          const sjAuth = effectiveHeaders['secure-join-auth'];
          const sjFingerprint = effectiveHeaders['secure-join-fingerprint'];
          const sjGroupMemberAdded = effectiveHeaders['chat-group-member-added'];

          if (secureJoinStep) {
            AppLog.info('SecureJoin', `Step detected: ${secureJoinStep}`, { from: peerEmail, auth: sjAuth, invite: sjInviteNumber });
          }

          if (secureJoinStep) {
            const step = secureJoinStep.trim().toLowerCase();
            AppLog.info('SecureJoin', `step received: ${step} from ${peerEmail}`);

            if (step === 'vc-request' || step === 'vg-request') {
              // ===== We are Alice (inviter/channel owner) — Step 3 =====
              // Someone scanned our QR code and sent a request.
              // Validate the invite number against our own tokens.

              // Check against our personal securejoin URI
              let isValidPersonalInvite = false;
              if (this.securejoinUri) {
                const hashIdx = this.securejoinUri.indexOf('#');
                if (hashIdx >= 0) {
                  const params = {};
                  this.securejoinUri.substring(hashIdx + 1).split('&').forEach((p, i) => {
                    if (i === 0) return; // fingerprint
                    const [k, v] = p.split('=');
                    params[k] = decodeURIComponent(v || '');
                  });
                  if (sjInviteNumber && (params.i === sjInviteNumber || params.j === sjInviteNumber)) {
                    isValidPersonalInvite = true;
                  }
                }
              }

              // Check against our channel invite numbers
              let matchedChannel = null;
              if (step === 'vg-request' && sjInviteNumber) {
                matchedChannel = this.channels.find(ch =>
                  ch.role === 'owner' && ch.inviteNumber === sjInviteNumber
                );
              }

              if (matchedChannel || isValidPersonalInvite) {
                contact.pendingVerification = true;
                if (matchedChannel) {
                  AppLog.success('SecureJoin', `Valid vg-request for channel "${matchedChannel.name}" from ${peerEmail}`);
                  this.toast('info', `🔐 ${peerEmail} wants to join #${matchedChannel.name}`);
                } else {
                  this.toast('info', `🔐 ${peerEmail} wants to verify with you`);
                }

                // Alice -> Bob: send vg-auth-required / vc-auth-required
                const authStep = step === 'vg-request' ? 'vg-auth-required' : 'vc-auth-required';
                try {
                  await this.sendSecureJoinStep(peerEmail, authStep);
                  AppLog.success('SecureJoin', `Sent ${authStep} to ${peerEmail}`);
                } catch (e) {
                  AppLog.error('SecureJoin', `Failed to send ${authStep}: ` + e.message, e.stack);
                }
              } else {
                console.warn(`Ignoring ${step}: bad invite number "${sjInviteNumber}"`);
              }

            } else if (step === 'vc-auth-required' || step === 'vg-auth-required') {
              // ===== We are Bob (joiner) — Step 4 =====
              // Alice acknowledged our request, now send request-with-auth
              AppLog.info('SecureJoin', `${step}: contact.securejoinAuth=${contact.securejoinAuth}, contact.pgpKey=${!!contact.pgpKey}`);

              // Find the auth token — may be on a different contact object due to address rotation/reset
              const authContact = this.contacts.find(c => c.securejoinAuth);
              if (authContact && authContact.email !== contact.email) {
                AppLog.info('SecureJoin', `Auth token found on different contact: ${authContact.email} vs ${contact.email}`);
                ...
                // CRITICAL: Rebind any pending channels created by the old address so Bob can group-join
                this.channels.forEach(ch => {
                  if (ch.createdBy === authContact.email) {
                    AppLog.info('SecureJoin', `Rebinding channel #${ch.name} creator to ${contact.email}`);
                    ch.createdBy = contact.email;
                  }
                });
                await this.saveState();
              }
              ...
              if (contact.securejoinAuth) {
                if (contact.pgpKey) {
                  ...
                  console.warn('Have auth token but no peer PGP key — cannot encrypt request-with-auth');
              ...
              // ===== We are Alice (inviter/owner) — Steps 5+6 =====
              // Bob sent auth + fingerprint. Validate, then confirm.
              console.log(`${step} from ${peerEmail}: auth=${sjAuth}, fp=${sjFingerprint}`);

              let authValid = false;
              let matchedChannel = null;

              // Check auth against our channels
              if (step === 'vg-request-with-auth' && sjAuth) {
                matchedChannel = this.channels.find(ch =>
                  ch.role === 'owner' && ch.auth === sjAuth
                );
                if (matchedChannel) {
                  authValid = true;
                  console.log(`Valid auth for channel "${matchedChannel.name}"`);
                }
              }

              // Check auth against personal securejoin
              if (!authValid && sjAuth && this.securejoinUri) {
                const hashIdx = this.securejoinUri.indexOf('#');
                if (hashIdx >= 0) {
                  const params = {};
                  this.securejoinUri.substring(hashIdx + 1).split('&').forEach((p, i) => {
                  ...
                  if (params.s === sjAuth) authValid = true;
                }
              }
              ...
              if (authValid) {
                ...
                if (sjFingerprint) contact.fingerprint = sjFingerprint.toUpperCase();

                if (matchedChannel) {
                  // Add joiner to channel members
                  if (!matchedChannel.members.includes(peerEmail)) {
                    matchedChannel.members.push(peerEmail);
                  }
                  this.toast('success', `✅ ${peerEmail} joined #${matchedChannel.name}`);

                  // Send vg-member-added to complete the handshake
                  try {
                    await this.sendSecureJoinMemberAdded(peerEmail, matchedChannel.id);
                    AppLog.success('SecureJoin', `Sent vg-member-added to ${peerEmail} for channel ${matchedChannel.name}`);
                  } catch (e) {
                    AppLog.error('SecureJoin', 'sendSecureJoinMemberAdded failed: ' + e.message, e.stack);
                  }
                } else {
                  // Personal verification — send vc-contact-confirm
                  this.toast('success', `✅ Verified contact: ${peerEmail}`);
                  try {
                    await this.sendSecureJoinStep(peerEmail, 'vc-contact-confirm');
                  } catch (e) {
                    AppLog.error('SecureJoin', 'sendSecureJoinStep vc-contact-confirm failed: ' + e.message, e.stack);
                  }
                }
              } else {
                AppLog.warn('SecureJoin', `${step}: invalid auth code from ${peerEmail}`);
              }

            } else if (step === 'vc-contact-confirm') {
              // ===== We are Bob — personal verification complete =====
              contact.verified = true;
              this.toast('success', `✅ Verified contact: ${peerEmail}`);

            } else if (step === 'vg-member-added') {
              // ===== We are Bob — channel join complete (Step 7) =====
              contact.verified = true;

              // Check if we have a channel that matches
              const addedAddr = sjGroupMemberAdded || '';
              const isSelfAdded = addedAddr.toLowerCase() === this.credentials.email.toLowerCase();

              if (isSelfAdded) {
                // We were added to a channel — find the matching channel
                const pendingChannel = this.channels.find(ch =>
                  ch.role === 'subscriber' && ch.createdBy === peerEmail
                );
                if (pendingChannel) {
                  pendingChannel.status = 'joined';
                  pendingChannel.verified = true;
                  this.selectChannel(pendingChannel); // Auto-open the channel
                  this.toast('success', `✅ Joined channel #${pendingChannel.name}`);
                } else {
                  this.toast('success', `✅ Added to group by ${peerEmail}`);
                }
              } else {
                this.toast('info', `${addedAddr || peerEmail} was added to the group`);
              }
            }

            // Save state after any SecureJoin step that modified data
            await this.saveState();
          }
        ...
        if (isDeleteRequest) {
          AppLog.info('Chat', `Chat-Delete received from ${peerEmail}: ${chatDeleteHeader}`);
        ...
        if (!isHidden && !isFromSelf) {
          // Check if this message belongs to a channel (broadcast)
          let matchedChannelId = null;

          // Primary: match by Chat-Group-ID (web client always sends this)
          const chatGroupId = effectiveHeaders['chat-group-id'];
          if (chatGroupId) {
            const matchCh = this.channels.find(ch => ch.id === chatGroupId.trim());
            if (matchCh) {
              matchedChannelId = matchCh.id;
              console.log(`Channel message matched by Chat-Group-ID: #${matchCh.name}`);
            }
          }

          // Fallback: match by Chat-Group-Name (Delta Chat core uses this for broadcasts)
          if (!matchedChannelId) {
            const chatGroupName = effectiveHeaders['chat-group-name'];
            if (chatGroupName) {
              const groupName = chatGroupName.trim();
              // Match by name + sender: check createdBy OR members list
              // (createdBy may have been rebound during SecureJoin address rotation)
              const matchCh = this.channels.find(ch =>
                ch.name === groupName && (
                  ch.createdBy === peerEmail ||
                  (ch.members || []).some(m => m.toLowerCase() === peerEmail)
                )
              );
              if (matchCh) {
                matchedChannelId = matchCh.id;
                console.log(`Broadcast message from ${peerEmail} matched to channel #${matchCh.name}`);
              }
            }
          }
          ...
          this.allMessages.push({
            ...
            peerEmail,
            imageData,
            channelId: matchedChannelId || null,
            rfc724mid: rawHeaders['message-id'] || null, // track for delete-for-everyone
          });

          if (matchedChannelId) {
            // Channel message — update unread on the channel
            const ch = this.channels.find(c => c.id === matchedChannelId);
            if (ch && this.selectedChannel?.id !== matchedChannelId) {
              ch.unread = (ch.unread || 0) + 1;
            }
            this.toast('info', `New message in #${ch?.name || 'channel'}`);
          } else if (this.selectedContact?.email === peerEmail) {
            this.$nextTick(() => this.scrollToBottom());
        ...
        AppLog.error('App', 'Failed to process message: ' + e.message, e.stack);
      }
    },
    ...
    // Send a generic SecureJoin step (auth-required, contact-confirm)
    // Alice -> Bob: plaintext (no encryption needed for these steps)
    async sendSecureJoinStep(toEmail, step) {
      const msgId = `<${this.generateId()}@${this.credentials.email.split('@')[1]}>`;
      const boundary = 'securejoin-' + this.generateId();
      const now = new Date().toUTCString();
    ...
    // Send vg-member-added (Alice -> Bob) — completes the channel join handshake
    // Mirrors securejoin.rs: Alice sends encrypted vg-member-added with Chat-Group-Member-Added
    async sendSecureJoinMemberAdded(toEmail, channelId) {
      const step = 'vg-member-added';
      const msgId = `<${this.generateId()}@${this.credentials.email.split('@')[1]}>`;
      const now = new Date().toUTCString();

      const contact = this.contacts.find(c => c.email === toEmail.toLowerCase());

      // Build raw message — try encrypted if we have the key
      const rawEmail = [
        this.buildFromHeader(),
        `To: <${toEmail}>`,
        `Date: ${now}`,
        ...
        `Chat-Version: 1.0`,
        `Secure-Join: ${step}`,
        `Chat-Group-Member-Added: ${toEmail}`,
      ];

      if (channelId) {
        rawEmail.push(`Chat-Group-ID: ${channelId}`);
      }
      ...
      if (contact?.pgpKey) {
        ...
        const encryptedBody = await this.encryptMessage(bodyText, contact.pgpKey, {
          ...
          extraHeaders: [
            `Secure-Join: ${step}`,
            `Chat-Group-Member-Added: ${toEmail}`,
            channelId ? `Chat-Group-ID: ${channelId}` : '',
          ].filter(Boolean).join('\r\n'),
        });

        rawEmail.push(
          `Content-Type: multipart/encrypted; protocol="application/pgp-encrypted"; boundary="pgp-member"`,
          `MIME-Version: 1.0`,
          '',
          '--pgp-member',
          'Content-Type: application/pgp-encrypted',
    ...
    toast(type, message) {
      AppLog.info('Toast', `[${type}] ${message}`);
      this.toasts.push({ id: Date.now(), type, message });
      // Self-destruct toast after 5s
      setTimeout(() => {
