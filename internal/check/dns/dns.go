...
import (
	...
	"github.com/themadorg/madmail/internal/check"
)
...
func requireMatchingRDNS(ctx check.StatelessCheckContext) module.CheckResult {
	...
	if ip := net.ParseIP(srcDomain); ip != nil {
		ctx.Logger.Debugf("source domain is an IP, skipping")
		return module.CheckResult{}
	}
...
func requireMXRecord(ctx check.StatelessCheckContext, mailFrom string) module.CheckResult {
	...
	if strings.HasPrefix(domain, "[") && strings.HasSuffix(domain, "]") {
		ctx.Logger.Debugf("domain is an IP literal, skipping")
		return module.CheckResult{}
	}
